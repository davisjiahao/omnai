...
import { runPacketSchema } from '../src/execution/packets.js';
import {
...
function writerPacket() {
  const withoutHash = {
    ...
    testCaseRefs: [{
      id: 'TC-0001',
      scope: { kind: 'PROJECT', project: 'quote', changeId: 'CHG-0001', revision: 'REV-0001' },
      contentHash: HASH_B,
    }],
  ...
  return { ...withoutHash, packetHash: hashObject(withoutHash) } as const;
}
...
function contractManifest() {
  return {
    ...
    participants: [
      ...
      { project: 'client', changeId: 'CHG-0001', revision: 'REV-0001', baseline: 'BL-0001', taskId: 'TASK-001', role: 'CONSUMER' },
    ],
...
test('materialized lifecycle cursors and execution events require possible hash links', () => {
  ...
  assert.equal(executionEventSchema.parse({ ...executionEvent(), aggregateType: 'commitset', machineVersion: 2 }).machineVersion, 2);
  assert.equal(executionEventSchema.parse({ ...executionEvent(), aggregateType: 'verification-plan', aggregateId: 'VPL-0001' }).aggregateType, 'verification-plan');
  assert.equal(executionEventSchema.parse({ ...executionEvent(), aggregateType: 'environment-run', aggregateId: 'IER-0001' }).aggregateType, 'environment-run');
});
...
test('Git object IDs accept exactly SHA-1 or SHA-256 widths', () => {
  ...
  const { packetHash: _packetHash, ...withoutHash } = base;
  const sha256WithoutHash = { ...withoutHash, git: { ...base.git, startingHead: 'f'.repeat(64) } };
  const sha256 = runPacketSchema.parse({ ...sha256WithoutHash, packetHash: hashObject(sha256WithoutHash) });
  ...
  for (const width of [41, 63]) {
    const changed = { ...withoutHash, git: { ...base.git, startingHead: 'f'.repeat(width) } };
    assert.throws(() => runPacketSchema.parse({ ...changed, packetHash: hashObject(changed) }), /startingHead|invalid/i);
  }
});
