...
父级 `AGENTS.md` 还必须约束 Codex 的解释方式，让跨领域协作保持准确且易读：

- 先给结论和白话解释，再引入正式术语；
- 专业术语或缩写第一次出现时，用一句话解释，同时保留正式名称以便搜索；
- 使用短句、具体名词和主动语态，通过可观察结果、取舍和用户影响说明方案；
- 根据用户在当前领域表现出的熟悉程度调整解释深度，不把一个领域的专业能力推定到其他领域；
- 只在视觉表达能明显降低理解成本时使用最小必要图示：精确比较优先用表格，流程、层级、状态或跨仓库关系优先用 Mermaid；不添加装饰性图示。

该契约不是禁止专业术语，也不能用含糊类比替代必要的技术细节。仓库内更具体的 `AGENTS.md` 可以补充领域规则；用户的直接要求可以调整某次回复的解释深度。首版不依赖外部 skill、私有渲染器或特定模型能力，无法使用更丰富的可视化能力时退回 Markdown 或 Mermaid。
...
10. 自动生成的父级 `AGENTS.md` 包含默认沟通契约：术语首次出现时解释，并只在有助理解时使用表格或 Mermaid。
