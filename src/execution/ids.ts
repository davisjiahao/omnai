...
export type ExecutionIdKind = 'contract' | 'wave' | 'run' | 'commitset' | 'evidence' | 'attention' |
  'verification-plan' | 'environment-run';
...
const definitions = {
  ...
  // Reserve both the new aggregate directory and the legacy flat YAML name.
  'verification-plan': ['verification-plans', 'VPL', /^VPL-(\d{4})(?:\.yaml)?$/],
  'environment-run': ['integration-environments/runs', 'IER', /^IER-(\d{4})$/],
